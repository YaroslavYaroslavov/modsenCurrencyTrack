...
(function (global, factory) {
  ...
  class CandlestickElement extends FinancialElement {
    draw(ctx) {
      ...
      if (close < open) {
        ...
        borderColor = "#14ca7f";
        ctx.fillStyle = helpers.valueOrDefault(
          me.color ? me.color.up : undefined,
          globalOpts$1.elements.candlestick.color.up
        );
      } else if (close > open) {
        borderColor = "#ec3842";
        // borderColor = helpers.valueOrDefault(
      ...
      ctx.strokeStyle = helpers.valueOrDefault(
        borderColor,
        globalOpts$1.elements.candlestick.borderColor
      );
  ...
  CandlestickElement.defaults = helpers.merge({}, [
  ...
  class OhlcElement extends FinancialElement {
    draw(ctx) {
      ...
      const armLengthRatio = helpers.valueOrDefault(
        me.armLengthRatio,
        globalOpts.elements.ohlc.armLengthRatio
      );
      let armLength = helpers.valueOrDefault(
        me.armLength,
        globalOpts.elements.ohlc.armLength
      );
      if (armLength === null) {
      ...
      if (close < open) {
        ctx.strokeStyle = helpers.valueOrDefault(
          me.color ? me.color.up : undefined,
          globalOpts.elements.ohlc.color.up
        );
      } else if (close > open) {
        ctx.strokeStyle = helpers.valueOrDefault(
          me.color ? me.color.down : undefined,
          globalOpts.elements.ohlc.color.down
        );
      } else {
        ctx.strokeStyle = helpers.valueOrDefault(
          me.color ? me.color.unchanged : undefined,
          globalOpts.elements.ohlc.color.unchanged
        );
      }
      ctx.lineWidth = helpers.valueOrDefault(
        me.lineWidth,
        globalOpts.elements.ohlc.lineWidth
      );
  ...
  OhlcElement.id = "ohlc";
  OhlcElement.defaults = helpers.merge({}, [
